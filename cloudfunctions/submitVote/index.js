...
function validateAndNormalize(event) {
  ...
  const groups = GROUPS.map((officialGroup) => {
    ...
    return { id: officialGroup.id, name: officialGroup.name, candidates, alternativeName, alternativeUnit };
  });

  return { voterName, voterUnit, groups };
}
...
exports.main = async (event) => {
  try {
    ...
    const duplicateQuery = await db.from(TABLE)
      .select('id')
      .eq('voter_name', normalized.voterName)
      .eq('voter_unit', normalized.voterUnit)
      .range(0, 0);

    if (duplicateQuery.error) throw new Error(duplicateQuery.error.message || '重复校验失败');
    if (Array.isArray(duplicateQuery.data) && duplicateQuery.data.length > 0) {
      return { ok: false, code: 'DUPLICATE', message: '该姓名和单位已经提交过本次投票，不能重复提交' };
    }

    const disagreeCount = normalized.groups.reduce(
      (sum, g) => sum + g.candidates.filter(c => c.vote === '不同意').length, 0
    );

    const voteData = {
      groups: normalized.groups,
      uid,
      source: 'cloudbase-postgresql-v1',
      version: 3
    };

    const inserted = await db.from(TABLE).insert({
      voter_name: normalized.voterName,
      voter_unit: normalized.voterUnit,
      vote_data: voteData,
      disagree_count: disagreeCount,
      client_time: cleanText(event.clientTime, 60)
    });

    if (inserted.error) throw new Error(inserted.error.message || '写入数据库失败');

    return { ok: true, disagreeCount };
  } catch (err) {
    console.error('submitVote failed', err);
    return { ok: false, code: 'SERVER_ERROR', message: err && err.message ? err.message : '服务器处理失败，请稍后重试' };
  }
};
