...
exports.main = async (event) => {
  try {
    ...
    const query = await db.from(TABLE)
      .select('id,created_at,voter_name,voter_unit,vote_data,disagree_count,client_time')
      .order('created_at', { ascending: false })
      .range(0, 999);

    if (query.error) throw new Error(query.error.message || '读取数据库失败');
    const docs = Array.isArray(query.data) ? query.data : [];
    ...
    for (const row of docs) {
      let voteData = row.vote_data || {};
      if (typeof voteData === 'string') {
        try { voteData = JSON.parse(voteData); } catch (_) { voteData = {}; }
      }

      for (const sg of (voteData.groups || [])) {
    ...
    const voters = docs.map((d, i) => ({
      ...
      name: d.voter_name || '',
      unit: d.voter_unit || '',
      disagreeCount: Number(d.disagree_count || 0),
      clientTime: d.client_time || '',
      createdAt: d.created_at || ''
    }));

    return { ok: true, totalVotes: docs.length, groups, voters, refreshedAt: new Date().toISOString() };
  } catch (err) {
